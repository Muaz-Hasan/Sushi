...
import styles from "./BlossomScene.module.scss";
...
class Petal {
  constructor(config) {
    ...
    this.el.className = `${styles.petal} ${this.customClass}`;
...
class BlossomScene {
  constructor(config) {
    ...
    if (!this.container) {
      throw new Error("[container] provided was not found in document");
    }
...
const BlossomSceneComponent = () => {
  const containerRef = useRef(null);

  useEffect(() => {
    const petalsTypes = [
      { customClass: styles["petal-style1"] },
      { customClass: styles["petal-style2"] },
      { customClass: styles["petal-style3"] },
      { customClass: styles["petal-style4"] },
    ];
    ...
    const blossomScene = new BlossomScene(myBlossomSceneConfig);
  ...
  return (
    <div
      ...
      className={`container mx-auto w-full min-h-screen flex justify-center items-center z-0 ${styles.container}`}
    ></div>
  );
};

export default BlossomSceneComponent;
